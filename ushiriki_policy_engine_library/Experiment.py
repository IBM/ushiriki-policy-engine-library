...
class Experiment():
    ...
    def _postJob(self, job, seed = None):
        ...
        try:
            ...
            data = json.dumps({"actions":interventionlist, "experimentId": self.experimentId, "jobSeeds": {"key": seed}});
    ...
    def _postBulkJobs(self, jobs, seeds = None):
        ...
        try:
            ...
            for job,seed in zip(jobs, seeds):
                ...
                for intervention in job:
                    interventionlist.append( {"modelName":intervention_names[int(intervention[0])],"coverage":intervention[2], "time":"%s"%int(intervention[3])} )
                data.append({"actions":interventionlist, "experimentId": self.experimentId, "jobSeeds": {"key": seed}, "locationId":self._locationId, "resolution":self._resolution, "userId":self._userId});
